...
public interface RetryMsgMapper extends BaseMapper<RetryMsg> {
    ...
    @Select("select count(1) from ems_retry_msg where state = 1 and group_name = #{group} and next_consumer_time < #{date} ")
    long count(@Param("date") Date date, @Param("groupName") String groupName);
}
